        ...
        if form.is_valid():
            ...
            if objTest.get_token() != False:
                ...
                for i in range(10):
                    ...
                    i += 1
                    if i == 9:
                        # Если невозможно получить положительный статус, рекомендуется проверить статус на сайте CDN
                        messages.error(request, "Невозможно получить положительный статус. Проверьте статус на CDN")
                        form = CacheForm()
                        return render(request, "test_cash/index.html", {'form': form})
